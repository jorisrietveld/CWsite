...
<div id="contentvak">
	<div class="ContentLeft" style="color: white;">
		<?php
			$query = $database->query("SELECT `value` FROM `".$config->dbinfo['prefix']."_pagina_variabel` where `paginaid`='" . $_GET['id'] . "' and `variabel_naam`='image'");
			$row = $database->fetch_assoc($query);
			echo "<img src='".$row['value']."' style='max-width: 190px;'>";
		?>
	</div>
	<div class="ContentRight">
		<?php
			$query = $database->query("SELECT `value` FROM `".$config->dbinfo['prefix']."_pagina_variabel` where `paginaid`='" . $_GET['id'] . "' and `variabel_naam`='text'");
			$row = $database->fetch_assoc($query);
			echo $row['value'];
		?>
	</div>
